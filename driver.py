...
async def post_most_recent_lines(blob, minutes):
    all_lines = blob.download_as_text().split('\n')
    half_index = len(all_lines) / 2
    half_index = int(half_index)
    first_half_lines = ""
    second_half_lines = ""
    for i in range(0, half_index):
        first_half_lines += all_lines[i] + "\n"

    for i in range(half_index, len(all_lines)):
        second_half_lines += all_lines[i] + "\n"
